...
<div class="bg-white rounded-lg shadow-md p-6">
    <h2 class="text-xl font-semibold text-azulOscuro mb-6">Editar Evento</h2>
    ...
    <form action="{{ route('public.eventos.update', $evento->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="descripcion" id="descripcion" rows="3" 
                          class="w-full border-gray-300 rounded-md shadow-sm focus:border-azul focus:ring focus:ring-azul/20">{{ old('descripcion', $evento->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="time" name="hora_inicio" id="hora_inicio" value="{{ old('hora_inicio', $evento->hora_inicio) }}" 
                       class="w-full border-gray-300 rounded-md shadow-sm focus:border-azul focus:ring focus:ring-azul/20" required>
                @error('hora_inicio')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="time" name="hora_fin" id="hora_fin" value="{{ old('hora_fin', $evento->hora_fin) }}" 
                       class="w-full border-gray-300 rounded-md shadow-sm focus:border-azul focus:ring focus:ring-azul/20" required>
                @error('hora_fin')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div class="mt-6 flex justify-end space-x-3">
            <form action="{{ route('public.eventos.destroy', $evento->id) }}" method="POST" class="inline-block">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700" 
                        onclick="return confirm('¿Estás seguro de querer eliminar este evento? Esta acción no se puede deshacer.')">
                    Eliminar
                </button>
            </form>
    ...
    <div class="mt-8 border-t border-gray-200 pt-6">
        <h3 class="font-medium text-lg mb-4">Acciones adicionales</h3>
        <div class="flex flex-col md:flex-row gap-4">
            <a href="{{ route('public.tomar-asistencia', $evento->id) }}" 
               class="px-4 py-2 bg-morado text-white rounded-md hover:bg-morado-dark inline-flex items-center justify-center">
                <svg class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                </svg>
                Tomar asistencia
            </a>
            <a href="{{ route('public.asistencias.evento', $evento->id) }}" 
               class="px-4 py-2 bg-azulOscuro text-white rounded-md hover:bg-azulOscuro-dark inline-flex items-center justify-center">
                <svg class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
